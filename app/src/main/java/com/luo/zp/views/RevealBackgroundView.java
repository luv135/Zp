...
public class RevealBackgroundView extends View {
    ...
    public void startFromLocation(int[] tapLocationOnScreen) {
        startFromLocation(tapLocationOnScreen,false);
    }
    public void startFromLocation(int[] tapLocationOnScreen,boolean reveal) {
        int start = 0;
        int end = getWidth()+getHeight();
        if(reveal){
            start = end;
            end =0;
        }
        ...
        revealAnimator = ObjectAnimator.ofInt(this, "currentRadius", start, end).setDuration(FILL_TIME);
